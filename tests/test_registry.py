...
def test_base_sepolia_present_and_enabled() -> None:
...
def test_lookup_by_x402_name_alias() -> None:
    assert get_network("base-sepolia") is REGISTRY["eip155:84532"]


def test_base_mainnet_present_but_disabled() -> None:
    n = get_network("eip155:8453")
    assert n is not None and not n.enabled


def test_unknown_network_returns_none() -> None:
    assert get_network("eip155:999999") is None


def test_enabled_networks_excludes_disabled() -> None:
